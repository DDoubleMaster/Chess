...
public class Bishop : MonoBehaviour
{
    ...
    private void OnMouseUp()
    {
		if (gameController.quene == figure.opponent)
		{
			...
			if (Mathf.Abs((float)Math.Round(oldParent.x, 2) - (float)Math.Round(newParent.x, 2)) == Mathf.Abs((float)Math.Round(oldParent.z, 2) - (float)Math.Round(newParent.z, 2)) && oldParent != newParent)
